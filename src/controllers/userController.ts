...
export const userInfo = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    sendResponse(
      res,
      200,
      sucessCallbackVal(code.successCode, db.data, '成功', true),
    );
  },
);
/**
...
export const findUserInfo = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    let result = null;
    ...
    sendResponse(
      res,
      200,
      sucessCallbackVal(code.successCode, result.data[0], '成功', true),
    );
  },
);
/**
 * @description 用户修改
 */
export const userEdit = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (id) {
      if (!iconUrl && !introduct && !username) {
        return sendResponse(
          res,
          400,
          sucessCallbackVal(code.failedCode, null, '至少修改一项', false),
        );
      }
      ...
      sendResponse(
        res,
        200,
        sucessCallbackVal(code.successCode, null, '成功', true),
      );
    } else {
      sendResponse(
        res,
        400,
        sucessCallbackVal(code.failedCode, null, 'id必传', false),
      );
    }
  },
);
...
export const upload = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const alyOssBucketUrl =
      'https://antd-simple-pro.oss-cn-beijing.aliyuncs.com/'; //oss中的Bucket 域名
    const ali_oss = {
    ...
    fs.rename(filePath, fileName, (err) => {
      ...
      if (err) {
        sendResponse(
          res,
          102,
          sucessCallbackVal(code.failedCode, err, '文件写入失败', false),
        );
      } else {
        ...
        co(function* () {
          ...
          const imageSrc =
            alyOssBucketUrl + (result as unknown as PutObjectResult).name; //自定义使用域名访问图片，（别忘记把域名解析至oss）
          ...
          sendResponse(
            res,
            102,
            sucessCallbackVal(code.failedCode, err, '上传失败', false),
          );
        });
      }
    });
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (db.code === code.successCode) {
      if (db.data.length) {
        ...
        res
          .status(200)
          .json(sucessCallbackVal(code.successCode, createToken, '成功', true));
      } else {
        res
          .status(200)
          .json(
      ...
      res
        .status(500)
        .json(sucessCallbackVal(code.failedCode, '服务器出错', '失败', false));
    }
  },
);
 ...
 * @description 文件下载
 */
export const fileDown = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    let datas = [['名字', '邮箱', '介绍']];
    const sqlStr = `select * from user`;
    const db = await mysqlDb.execute(sqlStr);
    if (db.code === code.successCode && db.data.length) {
      db.data.forEach((item: userInfoType) =>
        datas.push([item.username, item.email, item.introduct]),
      );
    }
    ...
    if (buffer.byteLength) {
      res
        .status(200)
        .json(sucessCallbackVal(code.successCode, buffer, '成功', true));
    } else {
      res
        .status(500)
        .json(sucessCallbackVal(code.failedCode, null, '下载失败', false));
    }
  },
);
