...
class AccessModule {
   ...
   */
  public async findMenuList(
    { page = 1, size = 20, sort = 'desc' }: pagationType,
    flag: isFindType = 'all',
  ) {
    //查询数据库
    let isLimit = null;
  ...
  public getCurrentUserMenuAuthTree = asyncHandler(
    async (req: Request, res: Response) => {
      const surroundings = process.env.NODE_ENV;
      let result = await this.findMenuList({});
      const arr = result.data;
      let fatherElement: menuAccessType[] = arr
        .filter((item: menuAccessType) => item.pid === 0)
        .map((item: menuAccessType) =>
          Object.assign({}, item, {
            createTime: new Date(item.createTime).getTime(),
          }),
        ); //取出父节点
      const renders = (
        fatherElement: menuAccessType[],
        arr: menuAccessType[],
      ) => {
        return fatherElement.map((item: menuAccessType) => {
          const childElement: menuAccessType[] = arr
            .filter((items: menuAccessType) => item.id === items.pid)
            .map((item: menuAccessType) =>
              Object.assign({}, item, {
                createTime: new Date(item.createTime).getTime(),
              }),
      ...
      sendResponse(
        ...
        sucessCallbackVal(
          code.successCode,
          surroundings === 'prod'
            ? fatherElement.filter((item) => item.id !== 18)
            : fatherElement,
          '成功',
          true,
        ),
  ...
  public getMenuList = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (result.data.length) {
      result.data = result.data.map((item: menuAccessType) =>
        Object.assign({}, item, {
          createTime: new Date(item.createTime).getTime(),
        }),
      );
    ...
    sendResponse(
      ...
      sucessCallbackVal(
        code.successCode,
        { list: result.data, total: findAllDataLength.data.length },
        '成功',
        true,
      ),
    );
    ...
    if (id) {
      ...
      if (!title || !url) {
        return sendResponse(
          res,
          400,
          sucessCallbackVal(code.failedCode, null, 'title,url必传', false),
        );
      }
  ...
  public deleteGetMenu = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!id) {
      return sendResponse(
        res,
        200,
        sucessCallbackVal(code.failedCode, null, 'id必须', false),
      );
    }
    ...
    sendResponse(
      res,
      200,
      sucessCallbackVal(code.successCode, null, '删除成功', true),
    );
  });
}
